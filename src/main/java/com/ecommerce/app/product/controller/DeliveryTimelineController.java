 ...
 */
@Controller
@RequestMapping("/deliverytimeline")
public class DeliveryTimelineController {
   ...
   DeliveryTimelineRepository deliveryTimelineRepository;
    ...
    public String add(Model model, @PathVariable Long pid) {
        ...
        return "product/deliveryoption/delivery_timeline";
    }

    @PostMapping("/save")
    @ResponseBody
    public void save(@Valid DeliveryTimeline deliveryTimeline, HttpServletResponse response) {
    ...
    public String deleteUnit(@PathVariable Long id, HttpServletResponse response) {
        String message;
        String messageType;
        if (!deliveryTimelineRepository.existsById(id)) {
