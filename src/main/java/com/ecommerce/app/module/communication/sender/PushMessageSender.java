...
public class PushMessageSender implements MessageChannelSender {
    ...
    public PushMessageSender(ProviderHttpClient providerHttpClient) {
        this.providerHttpClient = providerHttpClient;
    }

    @Override
    public boolean supports(MessageChannel channel) {
        return channel == MessageChannel.PUSH;
    }
    ...
    public CommunicationSendResult send(MessageDispatchRequest request, MessageProvider provider, RenderedMessage renderedMessage) {
        return providerHttpClient.post(request, provider, renderedMessage, "PUSH_SENT");
    }
}
