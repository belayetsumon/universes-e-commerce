...
import com.ecommerce.app.module.order.repository.PaymentRepository;
...
public class PaymentService {
    ...
    @Autowired
    private FraudPaymentCaptureGuard fraudPaymentCaptureGuard;
    ...
    public RefundResult refundPaidAmount(SalesOrder order, String paymentDetails) {
        ...
        FraudGuardResult fraudGuard = fraudPaymentCaptureGuard.checkRefundAllowed(order);
        if (!fraudGuard.isAllowed()) {
            throw new IllegalArgumentException(fraudGuard.getReason());
        }
    ...
    public RefundResult refundAmount(SalesOrder order, BigDecimal refundAmount, String paymentDetails) {
        ...
        fraudPostOrderMonitoringService.recordRefundReleased(order, refundableAmount, savedPayment.getTransactionId());
