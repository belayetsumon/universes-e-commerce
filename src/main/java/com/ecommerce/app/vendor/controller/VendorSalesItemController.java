...
@Controller
@RequestMapping("/vendor-order/items")
public class VendorSalesItemController {

    private final SalesItemReportService salesItemReportService;
    private final VendorUserContext vendorUserContext;

    public VendorSalesItemController(SalesItemReportService salesItemReportService, VendorUserContext vendorUserContext) {
    ...
    public String index(
        ...
        try {
            ...
            Long vendorId = activeVendor != null ? activeVendor.getId() : null;
            ...
            model.addAttribute("errorMessage", "Runtime error while loading vendor sales items: " + userFacingMessage(ex));
        }
        populateModel(model, salesItems, result, q, status, returnStatus, dateRange, fromDate, toDate, size);
        return "vendor/sales/items";
    }
